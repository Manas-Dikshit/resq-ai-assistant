...
const LiveEventsPanel = () => {
  ...
  // Merge real API events with local data
  const realEvents = [
    ...(data?.earthquakes || []),
    ...(data?.nasaEvents || []),
  ];
  ...
  const allEvents = [
    ...realEvents.map((e: any) => ({
      id: e.id,
      type: e.type as 'flood' | 'fire' | 'earthquake' | 'storm',
      title: e.title,
      description: e.description,
      severity: e.severity || e.magnitude / 8,
      affected: 0,
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-1.5">
                {d.affected > 0 && <span className="text-xs text-muted-foreground">{d.affected.toLocaleString()} affected</span>}
